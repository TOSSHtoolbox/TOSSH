function [Q_n_day_min, error_flag, error_str] = sig_Q_n_day_min(Q, t, n_day)
%Q_n_day_min calculates n day minimum of flow time series.
%   
%   INPUT
...
% calculate signature
Q_n_day_min = NaN(length(n_day),1);
for i=1:length(n_day)
    Q_n_day_min(i) = n_day(i)*min(movmean(Q,n_day(i))); % returns value in mm per chosen window n_day
end
